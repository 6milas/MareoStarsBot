...
# --- FLYER API İŞLEVİ ---
async def check_flyer_sponsors(user: User) -> list:
    not_subbed = []
    try:
        if FLYER_API_KEY:
            url = f"{FLYER_API_URL}/get_tasks"
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=5)) as response:
                    if response.status == 200:
                        ...
                        tasks = resp_json.get("result", [])
                        if tasks:
                            for task in tasks:
                                if isinstance(task, dict):
                                    signature = task.get("signature")
                                    t_url = task.get("url") or task.get("link")
                                    t_name = task.get("title", "Спонсор")
                                    
                                    if signature:
                                        check_url = f"{FLYER_API_URL}/check_task"
                                        check_payload = {
                                            "key": FLYER_API_KEY,
                                            "signature": signature
                                        }
                                        async with session.post(check_url, json=check_payload, headers=headers, timeout=aiohttp.ClientTimeout(total=5)) as check_resp:
                                            if check_resp.status == 200:
                                                check_data = await check_resp.json()
                                                if check_data.get("result") != "subscribed":
                                                    if t_url:
                                                        not_subbed.append({
                                                            'name': t_name,
                                                            'url': t_url,
...
async def run_full_subscription_check(user: User, bot: Bot) -> tuple[bool, list]:
    ...
    # 2. TGrass API, Flyer API, PiarFlow выполняются параллельно для скорости
    ...
    async def check_flyer():
        return await check_flyer_sponsors(user)

    async def check_piarflow():
        return await check_piarflow_sponsors(user)

    # Параллельный запуск всех спонсоров 
    results = await asyncio.gather(check_piarflow(), check_tgrass(), check_flyer(), return_exceptions=True)
...
# --- ARA YAZILIM (İСПРАВЛЕНА ЗАЩИТА ОТ СТАРЫХ КНОПКИ В МЕНЮ) ---
...
async def add_req_sub_get_private_link(message: Message, state: FSMContext, bot: Bot):
    ...
    if not private_link.startswith("https://t.me/"):
        err_msg = "❌ Неверный формат ссылки. Ссылка должна начинаться с <code>https://t.me/</code>"
        if prompt_message_id:
            await bot.edit_message_text(text=err_msg, chat_id=message.chat.id, message_id=prompt_message_id, reply_markup=get_back_to_admin_panel_kb())
        else:
            await message.answer(err_msg, reply_markup=get_back_to_admin_panel_kb())
    ...
    db.add_required_channel(data['req_channel_name'], data['req_channel_id'], private_link)
    await state.clear()
    success_msg = f"✅ Приватный канал <b>{data['req_channel_name']}</b> успешно добавлен!"
    if prompt_message_id:
        await bot.edit_message_text(text=success_msg, chat_id=message.chat.id, message_id=prompt_message_id, reply_markup=get_back_to_admin_panel_kb())
    else:
        await message.answer(success_msg, reply_markup=get_back_to_admin_panel_kb())
...
async def admin_list_req_subs(call: CallbackQuery):
    channels = db.get_required_channels()
    if not channels:
        await call.answer("📭 Нет обязательных каналов.", show_alert=True)
        return
    text = "🔗 <b>Обязательные каналы:</b>\n\n"
    builder = InlineKeyboardBuilder()
    for pk_id, name, channel_id, url in channels:
        text += f"▪️ <a href='{url}'>{name}</a> (ID: <code>{channel_id}</code>)\n"
...
async def admin_del_req_sub(call: CallbackQuery):
    try:
        pk_id = int(call.data.split("_")[-1])
    except ValueError:
        return
    db.delete_required_channel(pk_id)
    await call.answer("✅ Канал удален из обязательных.", show_alert=True)
    await admin_list_req_subs(call)

@dp.callback_query(F.data == "admin_download_db", IsChiefAdmin())
async def admin_download_db(call: CallbackQuery, bot: Bot):
    if not os.path.exists(DB_FILE):
        await call.answer("❌ Файл базы данных не найден.", show_alert=True)
        return
    await call.message.answer_document(FSInputFile(DB_FILE), caption="📦 Текущая резервная копия базы данных.")
    await call.answer()
...
async def admin_upload_db_start(call: CallbackQuery, state: FSMContext):
    ...
    text = "📥 <b>Загрузка базы данных</b>\n\n<blockquote>Отправьте файл базы данных (.db), чтобы заменить текущую.</blockquote>"
    prompt_message = await call.message.edit_text(text, reply_markup=get_back_to_admin_panel_kb())
    if prompt_message:
        await state.update_data(prompt_message_id=prompt_message.message_id)
    await call.answer()
...
async def admin_upload_db_process(message: Message, state: FSMContext, bot: Bot):
    ...
    if not document.file_name.endswith('.db'):
        await message.answer("❌ Неверный формат файла. Требуется файл с расширением <code>.db</code>.", reply_markup=get_back_to_admin_panel_kb())
        return
...
async def main():
    ...
    print("Bot started successfully with Flyer API integration!")
