...
def get_admin_kb(user_id: int):
    ...
    builder.button(text="Статистика TGrass", callback_data="admin_stats", icon_custom_emoji_id=EMOJI_IDS["stats_tgrass"], style="primary")
    builder.button(text="Статистика PiarFlow", callback_data="admin_stats", icon_custom_emoji_id=EMOJI_IDS["stats_piarflow"], style="primary")
...
async def download_db_handler(call: CallbackQuery):
    ...
    await call.message.answer_document(db_file, caption="📦 Бэкап базы данных.")
    await call.answer()
...
async def upload_db_start(call: CallbackQuery, state: FSMContext):
    ...
    prompt = await call.message.edit_text("📥 Отправьте файл базы данных (.db), чтобы восстановить её.", reply_markup=get_back_to_admin_panel_kb())
    await state.update_data(prompt_message_id=prompt.message_id)
    await call.answer()
...
async def upload_db_process(message: Message, state: FSMContext, bot: Bot):
    ...
    if not message.document.file_name.endswith('.db'):
        prompt = await message.answer("❌ Отправьте файл с расширением .db.", reply_markup=get_back_to_admin_panel_kb())
        await state.update_data(prompt_message_id=prompt.message_id)
        return
    file_id = message.document.file_id
    file = await bot.get_file(file_id)
    await bot.download_file(file.file_path, DB_FILE)
    await message.answer("✅ База данных успешно восстановлена!", reply_markup=get_back_to_admin_panel_kb())
    await state.clear()
