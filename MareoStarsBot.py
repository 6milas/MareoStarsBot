...
EMOJI_IDS = {
    ...
    "stats_tgrass": "5936143551854285132",
    "stats_piarflow": "5936143551854285132",
...
def get_admin_kb(user_id: int):
    ...
    builder.button(text="Статистика TGrass", callback_data="admin_stats_tgrass", icon_custom_emoji_id=EMOJI_IDS["stats_tgrass"], style="primary")
    builder.button(text="Статистика PiarFlow", callback_data="admin_stats_piarflow", icon_custom_emoji_id=EMOJI_IDS["stats_piarflow"], style="primary")
...
# --- STATS API İŞLEVLERİ ---
async def get_tgrass_statistics() -> dict:
    try:
        url = "https://tgrass.space/stats"
        headers = {"accept": "application/json", "Auth": TGRASS_API_KEY}
...
async def get_piarflow_statistics() -> dict:
    try:
        url = f"{PIARFLOW_API_URL}/stats"
        headers = {
...
async def add_req_sub_get_name(message: Message, state: FSMContext, bot: Bot):
    ...
    prompt_text = "<b>Шаг 2/3: Telegram ID или Юзернейм</b>\n\n<blockquote>Введите ID канала (например: <code>-100123456789</code>) или юзернейм (например: <code>@channel</code>).</blockquote>"
    prompt = await message.answer(prompt_text, reply_markup=get_back_to_admin_panel_kb())
    await state.update_data(prompt_message_id=prompt.message_id)
...
async def add_req_sub_get_entry(message: Message, state: FSMContext, bot: Bot):
    await message.delete()
    await clean_prompt(state, message.chat.id, bot)
    await state.update_data(req_channel_id=message.text.strip())
    await state.set_state(AdminStates.add_req_private_link)
    prompt_text = "<b>Шаг 3/3: Ссылка на канал</b>\n\n<blockquote>Введите ссылку для входа в канал (например: <code>https://t.me/channel</code>).</blockquote>"
    prompt = await message.answer(prompt_text, reply_markup=get_back_to_admin_panel_kb())
    await state.update_data(prompt_message_id=prompt.message_id)
...
async def add_req_sub_get_private_link(message: Message, state: FSMContext, bot: Bot):
    ...
    await clean_prompt(state, message.chat.id, bot)
    data = await state.get_data()
    name = data.get("req_channel_name")
    channel_id = data.get("req_channel_id")
    url = message.text.strip()
    
    try:
        db.add_required_channel(name, channel_id, url)
        await message.answer(f"✅ Канал <b>{html.quote(name)}</b> успешно добавлен!", reply_markup=get_back_to_admin_panel_kb())
    except Exception as e:
        logging.error(f"Error adding channel: {e}")
        await message.answer("❌ Ошибка при добавлении канала (возможно, такой ID уже существует).", reply_markup=get_back_to_admin_panel_kb())
    finally:
        await state.clear()
...
async def list_req_subs(call: CallbackQuery):
    ...
    if not channels:
        await call.answer("📭 Список обязательных каналов пуст.", show_alert=True)
        return
    text = "🔗 <b>Обязательные каналы:</b>\n\n"
    builder = InlineKeyboardBuilder()
    for channel_pk_id, name, channel_id, url in channels:
        text += f"▪️ <b>{html.quote(name)}</b> | <code>{channel_id}</code>\n"
        builder.button(text=f"Удалить {name}", callback_data=f"admin_delete_req_sub_{channel_pk_id}", icon_custom_emoji_id=EMOJI_IDS["remove"], style="danger")
    builder.button(text="Назад", callback_data="admin_req_subs", icon_custom_emoji_id=EMOJI_IDS["back"], style="danger")
    builder.adjust(1)
    await call.message.edit_text(text, reply_markup=builder.as_markup())
    await call.answer()

@dp.callback_query(F.data.startswith("admin_delete_req_sub_"), IsAdmin())
async def delete_req_sub_handler(call: CallbackQuery):
    channel_pk_id = int(call.data.split("_")[-1])
    db.delete_required_channel(channel_pk_id)
    await call.answer("✅ Канал удален.", show_alert=True)
    await list_req_subs(call)

@dp.callback_query(F.data == "admin_download_db", IsAdmin())
async def admin_download_db_handler(call: CallbackQuery):
    if not os.path.exists(DB_FILE):
        await call.answer("❌ База данных не найдена.", show_alert=True)
        return
    try:
        db_file = FSInputFile(DB_FILE)
        await call.message.answer_document(db_file, caption="📁 Актуальная база данных.")
        await call.answer()
    except Exception as e:
        logging.error(f"Error downloading DB: {e}")
        await call.answer("❌ Ошибка при скачивании базы данных.", show_alert=True)
...
async def admin_upload_db_start(call: CallbackQuery, state: FSMContext):
    ...
    prompt = await call.message.edit_text(
        "📤 Отправьте файл базы данных (`.db` или `.sqlite`).",
        reply_markup=get_back_to_admin_panel_kb()
    )
    await state.update_data(prompt_message_id=prompt.message_id)
    await call.answer()
...
async def admin_upload_db_process(message: Message, state: FSMContext, bot: Bot):
    ...
    if not (document.file_name.endswith('.db') or document.file_name.endswith('.sqlite')):
        await message.answer("❌ Файл должен иметь расширение .db или .sqlite", reply_markup=get_back_to_admin_panel_kb())
        return
    ...
    try:
        ...
        await message.answer(f"❌ Ошибка при загрузке базы данных: {e}", reply_markup=get_back_to_admin_panel_kb())
    finally:
        await state.clear()

# --- WEB SERVER & BOT LAUNCH ---
app = Flask('')
